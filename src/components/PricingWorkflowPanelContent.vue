    ...
    <div class="flex-1 overflow-y-auto p-6 custom-scrollbar">
      ...
      <div v-else>
        <div class="max-w-2xl mx-auto py-4">
          <a-timeline v-if="props.channel.submissionHistory?.pricing?.date">
            <a-timeline-item color="green">
              <template #dot><div class="w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-white shadow-sm"></div></template>
              <div class="flex flex-col gap-2">
                <div class="text-[15px] font-black text-slate-800">{{ props.channel.submissionHistory.pricing.date }}</div>
                <div class="p-5 rounded-2xl border border-slate-100 bg-slate-50/50">
                  <div class="space-y-2 text-[13px] text-slate-600 font-medium">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-slate-900 font-bold">{{ props.channel.submissionHistory.pricing.user || 'N/A' }}</span>
                    </div>
                    <div v-if="props.channel.submissionHistory.pricing.notes" class="flex items-start gap-2">
                      <span class="text-slate-400 shrink-0">Remarks:</span>
                      <span class="text-slate-900 italic">"{{ props.channel.submissionHistory.pricing.notes }}"</span>
                    </div>
                  </div>
                </div>
              </div>
