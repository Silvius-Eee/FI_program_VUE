...
const legalFormCopy = computed(() => {
  ...
  return {
    entityLabel: 'Select Contracting Entities (Our Side)',
    uploadLabel: 'Draft Document Upload',
    uploadHint: `Click or drag ${legalDocumentTitle.value} draft here`,
    linkLabel: 'Document Link (Optional)',
    linkPlaceholder: 'https://docs.google.com/d/...',
    remarksLabel: 'Context / Remarks for Legal',
    remarksPlaceholder: 'E.g., Please review the governing law clause...',
  };
});
  ...
  >
    <div class="h-full flex flex-col bg-white">
      ...
      <div class="px-6 py-5 border-b border-slate-50 flex items-center gap-4">
        ...
        <span class="text-[18px] font-black text-slate-900 uppercase tracking-tight">Submit {{ legalDocumentTitle }} to Legal Department</span>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-8 custom-scrollbar">
        <div v-if="activeKey === 'submission'">
          <a-form layout="vertical">
            <a-form-item required>
              <template #label>
                <span class="text-[13px] font-bold text-slate-800 uppercase tracking-wider">
                  <span class="text-red-500 mr-1">*</span>{{ legalFormCopy.entityLabel }}
                </span>
              </template>
            ...
            <a-form-item class="mt-8">
              <template #label>
                <span class="text-[13px] font-bold text-slate-800 uppercase tracking-wider">{{ legalFormCopy.uploadLabel }}</span>
              </template>
              <a-upload-dragger class="bg-slate-50/50 border-dashed border-slate-200 rounded-2xl py-10 mt-2">
                ...
                <p class="text-[15px] font-bold text-slate-800 mb-1">{{ legalFormCopy.uploadHint }}</p>
              </a-upload-dragger>
            </a-form-item>

            <a-form-item class="mt-8">
              <template #label>
                <span class="text-[13px] font-bold text-slate-800 uppercase tracking-wider">{{ legalFormCopy.linkLabel }}</span>
              </template>
              <a-input 
            ...
            <a-form-item class="mt-8 mb-0" required>
              <template #label>
                <span class="text-[13px] font-bold text-slate-800 uppercase tracking-wider">
                  <span class="text-red-500 mr-1">*</span>{{ legalFormCopy.remarksLabel }}
                </span>
              </template>
              <a-textarea 
        ...
        <div v-else>
          <div class="max-w-2xl mx-auto py-4">
            <div v-if="channel.submissionHistory?.[docType.toLowerCase()]?.date">
              <a-timeline>
                <a-timeline-item color="green">
                  <template #dot><div class="w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-white shadow-sm"></div></template>
                  <div class="flex flex-col gap-2">
                    <div class="text-[15px] font-black text-slate-800">{{ channel.submissionHistory[docType.toLowerCase()].date }}</div>
                    <div class="p-5 rounded-2xl border border-slate-100 bg-slate-50/50">
                      <div class="space-y-2 text-[13px] text-slate-600 font-medium">
                        <div class="flex items-center gap-2">
                          ...
                          <span class="text-slate-900 font-bold">{{ channel.submissionHistory[docType.toLowerCase()].user || 'N/A' }}</span>
                        </div>
                        <div v-if="channel.submissionHistory[docType.toLowerCase()].notes" class="flex items-start gap-2">
                          <span class="text-slate-400 shrink-0">Remarks:</span>
                          <span class="text-slate-900 italic">"{{ channel.submissionHistory[docType.toLowerCase()].notes }}"</span>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <div class="p-6 border-t border-slate-50 bg-white flex justify-end gap-3">
        ...
        <a-button type="primary" @click="handleSubmit" class="h-[44px] px-8 rounded-xl font-bold bg-indigo-600 border-none shadow-md">Send {{ legalDocumentTitle }} Request</a-button>
      </div>
    </div>
  </a-drawer>
</template>
