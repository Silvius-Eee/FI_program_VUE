  ...
  >
    <div class="h-full flex flex-col bg-white">
      ...
      <div class="px-6 py-5 border-b border-slate-50 flex items-center gap-4">
        ...
        <span class="text-[18px] font-black text-slate-900">Submit Materials to Compliance</span>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-8 custom-scrollbar">
        <div v-if="activeKey === 'submission'">
          <a-form layout="vertical">
            ...
            <a-form-item class="mt-8">
              <template #label>
                <span class="text-[13px] font-bold text-slate-800 uppercase tracking-wider">Our KYC Package Upload</span>
              </template>
                          <a-upload-dragger class="bg-slate-50/50 border-dashed border-slate-200 rounded-2xl py-10 mt-2">
                            ...
                            <p class="text-[15px] font-bold text-slate-800 mb-1">Upload our business license, director IDs, etc.</p>
                          </a-upload-dragger>
            </a-form-item>

            <a-form-item class="mt-8">
              <template #label>
                <span class="text-[13px] font-bold text-slate-800 uppercase tracking-wider">Document Link (Optional)</span>
              </template>
              <a-input 
            ...
            <a-form-item class="mt-8 mb-0" required>
              <template #label>
                <span class="text-[13px] font-bold text-slate-800 uppercase tracking-wider">
                  <span class="text-red-500 mr-1">*</span>Additional Notes
                </span>
              </template>
              <a-textarea 
        ...
        <div v-else>
          <div class="max-w-2xl mx-auto py-4">
            <a-timeline v-if="channel.submissionHistory?.kyc?.date">
              <a-timeline-item color="green">
                <template #dot><div class="w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-white shadow-sm"></div></template>
                <div class="flex flex-col gap-2">
                  <div class="text-[15px] font-black text-slate-800">{{ channel.submissionHistory.kyc.date }}</div>
                  <div class="p-5 rounded-2xl border border-slate-100 bg-slate-50/50">
                    <div class="space-y-2 text-[13px] text-slate-600 font-medium">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-slate-900 font-bold">{{ channel.submissionHistory.kyc.user || 'N/A' }}</span>
                      </div>
                      <div v-if="channel.submissionHistory.kyc.notes" class="flex items-start gap-2">
                        <span class="text-slate-400 shrink-0">Remarks:</span>
                        <span class="text-slate-900 italic">"{{ channel.submissionHistory.kyc.notes }}"</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div v-else class="py-12 text-center">
              ...
              <p class="text-slate-400 font-medium">No KYC submission history yet.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 border-t border-slate-50 bg-white flex justify-end gap-3">
        ...
        <a-button type="primary" @click="handleSubmit" class="h-[44px] px-8 rounded-xl font-bold bg-[#0284c7] border-none shadow-md">Send KYC Package</a-button>
      </div>
    </div>
  </a-drawer>
</template>
